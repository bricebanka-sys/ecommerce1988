...
-- Suppression des tables existantes pour repartir d'un état propre
DROP TABLE IF EXISTS Brand;
DROP TABLE IF EXISTS Type;
DROP TABLE IF EXISTS Product;

-- Création de la table Brand (Marque)
CREATE TABLE `Brand` (
                         `Id` INT AUTO_INCREMENT PRIMARY KEY,
                         `Name` VARCHAR(255) NOT NULL
);

-- Inserer les donnes dans la Brand table
...
-- Création de la table Type (Catégorie de produit)
CREATE TABLE `Type` (
                        `Id` INT AUTO_INCREMENT PRIMARY KEY,
                        `Name` VARCHAR(255) NOT NULL
);

-- Insertion des Types (Types / Catégories)
...
-- Création de la table Product avec clés étrangères (Relations)
...
-- Insertion des data dans la table Product
